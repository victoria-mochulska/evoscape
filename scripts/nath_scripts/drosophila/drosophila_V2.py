# changing the way the vector field is calculated to make it analytical and not use the gradient and skewgradient operator
# Also to change the way the vector field FROM the data is calculated, make it more straightforward and note make a for loop.
# worked
...
@jax.jit 
def cost_func3(params, X, Y, Z, mask):
...
@jax.jit 
def cost_func2(params, X, Y, Z, mask):
    ...
    abc = jnp.mean(loss_magnitude_mask) #/ jnp.sum(mask)
    deg = jnp.mean(loss_angle_mask) #/ jnp.sum(mask)
...
gradient_vector_mask = JAXflow_grad_mixture(xx_mask, yy_mask, params[0])
rotation_vector_mask = JAXflow_rot_mixture(xx_mask, yy_mask, params[1])
