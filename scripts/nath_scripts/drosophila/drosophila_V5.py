...
# losses are now semilog
# sigma losses is now weighted
# reduced resolution of the error 
...
final, traj = make_trajectories(params, all_coords, ndt=1)
final2, traj2 = make_trajectories(params, all_coords, ndt=100)
...
with PdfPages("figures/my_plots_" + str(hihi) + "_" + str(1) + "_" + str(1) + ".pdf") as pdf:
   ...
   # --- First page: text summary ---
    ...
    fig = show_trajectories(all_coords[:,::skip_time,::skip_space], traj2[:, ::skip_time,::skip_space])
    pdf.savefig()
    plt.close(fig)
