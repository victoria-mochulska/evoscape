# making it optimized on only data, real data. Other version were using griddata and interpolated some of the data (because i thought it was okay ???).
# So we directly input the vectors calculted simply by the time difference for all data points.
# making the loss/cost_function more compact with function for magnitude and other things
# adding back verbose to see the evolution of the loss with epoch
...
vect_all = np.roll(all_coords, -1, axis=1) - all_coords
vect_all[:, -1,:] = 0
...
"""best_params_grad = jnp.array([[ 0.02603536, -0.11498541,  0.40018795,  0.29328132,  0.21709941,  0.85004969]])
best_params_rot = jnp.array([[-0.32947792, -0.09359128,  0.52065184,  0.32163528,  0.17626963,  3.24916719]])
best_loss = cost_func((best_params_grad, best_params_rot), X_all, Y_all, vect_all)"""
...
with PdfPages("figures/my_plots_" + str(hihi) + "_" + str(1) + "_" + str(1) + ".pdf") as pdf:
   ...
   # --- First page: text summary ---
    ...
    show_fields(X_all, Y_all, fitted_vect_field, vect_all, "Fitted vector field", "Original vector field", nbr_arrows=100)
    pdf.savefig()
    plt.close()


    error_on_vect_field = error(fitted_vect_field, vect_all)
    #print(round(np.max(error_on_vect_field), 2))

    show_error_display(X_all, Y_all, error_on_vect_field, "Error between original vector field and gaussians")
    pdf.savefig()
    plt.close()
    ...
    plt.scatter(all_coords[0], all_coords[1], s=0.05)
    axis_limit = 2
    ...
    for loss in range(times*cpu):
        plt.plot(loss_evol, evol[loss], color= "grey")
