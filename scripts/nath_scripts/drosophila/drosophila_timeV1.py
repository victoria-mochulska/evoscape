...
parser.add_argument("--derivative_importance", type=float, default=1)
...
derivative_importance = args.derivative_importance
...
init_conditions_grad = jax.vmap(random_init_time, in_axes=(0, None, None, None, None, None, None))(seeds, nbr_gaussians_grad, xx, yy, max_grad, min_grad, all_coords_trimmed.shape[1])
init_conditions_grad_sharded = jax.device_put(init_conditions_grad, sharding)
...
init_conditions_rot = jax.vmap(random_init_time, in_axes=(0, None, None, None, None, None, None))(seeds, nbr_gaussians_rot, xx, yy, max_rot, min_rot, all_coords_trimmed.shape[1])
init_conditions_rot_sharded = jax.device_put(init_conditions_rot, sharding)
...
"""losses, params_grad, params_rot, evol, params_grad_history, params_rot_history, loss_history = train_verbose_optax_time(X_all_trimmed, Y_all_trimmed, vect_all_trimmed, learning_rate, init_conditions_grad_sharded, init_conditions_rot_sharded, derivative_importance, MAXITER, verbose_dt)
params_grad.block_until_ready()
print("first run done")
losses, params_grad, params_rot, evol, params_grad_history, params_rot_history, loss_history = train_verbose_optax_traj_rand_time(X_all_trimmed, Y_all_trimmed, vect_all_trimmed, lr, percent, params_grad, params_rot, jnp.swapaxes(all_coords_trimmed, 0, 2), seed, length, derivative_importance, 500, verbose_dt)
print(f"Time with verbose :{time.time() - start_time:.2f}")
"""
losses, params_grad, params_rot, evol, params_grad_history, params_rot_history, loss_history = train_verbose_optax_traj_rand_time(X_all_trimmed, Y_all_trimmed, vect_all_trimmed, lr, percent, init_conditions_grad_sharded, init_conditions_rot_sharded, jnp.swapaxes(all_coords_trimmed, 0, 2), seed, length, derivative_importance, MAXITER, verbose_dt)
print(f"Time with verbose :{time.time() - start_time:.2f}")
...
final, traj = make_trajectories_time_dependance(params, all_coords, ndt=100)
...
show_animated_potentials(xx, yy, jnp.swapaxes(params_grad, 0, 1), jnp.swapaxes(params_rot, 0, 1), "Best fit grad animated", "Best fit rot animated", run_dir / "animated_potentials.gif")
show_animated_field(xx, yy, jnp.swapaxes(params_grad, 0, 1), jnp.swapaxes(params_rot, 0, 1), "Best fit vector field animated", run_dir / "animated_fields_combined.gif")
#show_animated_streamplot(xx, yy, jnp.swapaxes(params_grad, 0, 1), jnp.swapaxes(params_rot, 0, 1), "Best fit streamplot animated", run_dir / "animated_streamplots_combined.gif")

  

fitted_vect_field = prediction_of_vectors(X_all, Y_all, params)
...
loss_change_params = (minimal_param_change_velocity(params) + minimal_param_change_acceleration(params)) * derivative_importance
...
print("Loss of derivative of parameters :", loss_change_params)
print()
...
results = {
    ...
    "loss_change_params": float(loss_change_params),
...
with PdfPages(run_dir / "myplots.pdf") as pdf:
   ...
   # --- First page: text summary ---
    ...
    text_lines = [
        ...
        f"Loss change of parameters : {loss_change_params}",
    ...
    i = 0
    for gaussian_grad in params_grad:

        fig = plot_parameters_over_time(gaussian_grad, str(i))
        pdf.savefig()
        plt.close(fig)
        i+=1
    i = 0
    for gaussian_rot in params_rot:

        fig = plot_parameters_over_time(gaussian_rot, str(i))
        pdf.savefig()
        plt.close(fig)
        i+=1
